...
export default function ContractPageTenant() {
  const tenant = JSON.parse(localStorage.getItem('user') || 'null')
  ...
  const contractsByUser = contracts.filter((c) => c.tenant?.userId === tenant?.userId)

  const filteredContracts = contractsByUser.filter(
    (c) =>
    ...
    <Container className='flex flex-col gap-8 py-8 animate-in fade-in duration-700'>
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        <div className='bg-blue-50/50 border border-blue-100 p-4 rounded-2xl'>
          ...
          <p className='text-2xl font-black text-blue-900'>{contractsByUser.length}</p>
        </div>
        <div className='bg-emerald-50/50 border border-emerald-100 p-4 rounded-2xl'>
          <p className='text-xs font-bold text-emerald-500 uppercase'>Đang ở</p>
          <p className='text-2xl font-black text-emerald-900'>
            {contractsByUser.filter((c) => c.status === 'active').length}
          </p>
        </div>
      </div>
